...
public class ObjectStore {
  ...
  public void put(Device device, ResourceOperation operation, String value) {
    ...
    List<RestObject> objectsList = createObjectsList(operation, device);
    String deviceId = device.getId();
    List<Reading> readings = new ArrayList<>();

    for (RestObject obj: objectsList) {
  ...
  private List<RestObject>
      createObjectsList(ResourceOperation operation, Device device) {
    Map<String, RestObject> objects = profiles.getObjects().get(device.getName());
    List<RestObject> objectsList = new ArrayList<RestObject>();

    if (operation != null && objects != null) {
      RestObject object = objects.get(operation.getObject());
  ...
  private String transformResult(String result, RestObject object,
      Device device, ResourceOperation operation) {
