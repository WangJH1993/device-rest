...
public class CoreDataMessageHandler {
  ...
  private void updateLastConnected(String deviceName) {
    ...
    if (device != null) {
      ...
      logger.debug("No device found for device name: "
          + deviceName + ". Could not update last connected time");
    }
  }

  public List<ResponseObject> sendCoreData(String deviceName, List<Reading> readings,
      Map<String, RestObject> objects) {
    ...
    try {
      ...
      if (objects != null) {
        ...
        logger.debug("No profile object found for the device "
            + deviceName + ".  MQTT message ignored.");
      }
    } catch (Exception e) {
