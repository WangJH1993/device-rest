...
import org.edgexfoundry.domain.RestObject;
...
import org.edgexfoundry.rest.DeviceDiscovery;
import org.edgexfoundry.rest.ObjectTransform;
...
public class RestHandler {
  ...
  private static final EdgeXLogger logger =
      EdgeXLoggerFactory.getEdgeXLogger(RestHandler.class);
  ...
  @Value("${rest.device.init:#{null}}")
  private String restInit;
  @Value("${rest.device.init.args:#{null}}")
  private String restInitArgs;
  @Value("${rest.device.remove:#{null}}")
  private String restRemove;
  @Value("${rest.device.remove.args:#{null}}")
  private String restRemoveArgs;
...
//    if (restInit != null && commandExists(device, restInit)) {
//      executeCommand(device, restInit, restInitArgs);
//    }
//
...
//    if (restRemove != null && commandExists(device, restRemove)) {
//      executeCommand(device, restRemove, restRemoveArgs);
//    }
//
...
//    new Thread(() -> driver.process(device, cmd, arguments)).start();
//    return null;
//  }
...
//    Map<String, RestObject> objects = profiles.getObjects().get(deviceName);
//
...
//      RestObject object =
//          getRestObject(objects, objectName, transactionId);
//
...
//  }
  ...
  private RestObject getRestObject(Map<String,
      RestObject> objects, String objectName, String transactionId) {
    RestObject object = objects.get(objectName);
...
//      RestObject object, Map<String, RestObject> objects) {
//
